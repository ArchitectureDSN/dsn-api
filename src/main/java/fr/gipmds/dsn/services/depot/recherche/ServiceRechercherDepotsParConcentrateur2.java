package fr.gipmds.dsn.services.depot.recherche;
...
@NoCache
@Formatted
public class ServiceRechercherDepotsParConcentrateur2 {
    ...
    public Response listerRetours(
        ...
        try {
            final int plageDeRecherche; // Initialisation dans le bloc de verification de l'authentification
            // Parse dates
            Date dateDebut = null;
            Date dateFin = null;
            try {
            ...
            if (headerMap.get("jeton") != null) {
                plageDeRecherche = Config.plageDeRechercheParDeclarant;
            ...
            // Traitement pour concentrateur
                ...
                plageDeRecherche = Config.plageDeRechercheParConcentrateur;
            ...
            cal.add(Calendar.MINUTE, plageDeRecherche);
            if (dateFin.after(cal.getTime()))
                return Response.status(429).build();

            // 200
            final RechercherDepotParConcentrateurResponse entity = (RechercherDepotParConcentrateurResponse) JAXBContext.newInstance(RechercherDepotParConcentrateurResponse.class)
                    .createUnmarshaller()
                    .unmarshal(new StringReader(TestData.reponseRechercheListerDepots));
