...
@DisplayName("Rechercher Depots Par Concentrateur Test")
class RechercherDepotsParConcentrateurTest extends BaseTestClass {
    ...
    void rechercher422PlageMauvaisDebutTest() {
        ...
        ResteasyWebTarget target = client.target(getServerUrl() + "/lister-depots/1.0/120000/20130101130000");
        Builder builder = target.request();
        Invocation invocation = builder.buildGet();
        // When
        Response response = invocation.invoke();
    ...
    void rechercher200DebutDePlageSeulementTest() {
        // Given
        ResteasyClient client = new ResteasyClientBuilder().disableTrustManager().build();
        String date = getDateDebutFormatted(Config.plageDeRechercheParConcentrateur);
    ...
    void rechercher200Test() {
        // Given
        ResteasyClient client = new ResteasyClientBuilder().disableTrustManager().build();
        ResteasyWebTarget target = client.target(getServerUrl() + "/lister-depots/1.0/20130101120000/20130101121000");
        Builder builder = target.request();
        Map<String, String> map = new HashMap<>();
        map.put("concentrateur", TestData.concentrateurInscrit.getFauxJeton());
        map.put("declarant", Base64Utils.encode(TestData.declarantInscrit.getSNP()));
    ...
    void rechercher401AUTHORIZATIONMauvaisTest() {
        ...
        ResteasyWebTarget target = client.target(getServerUrl() + "/lister-depots/1.0/20130101120000/20130101121000");
        Builder builder = target.request();
        builder.header("Authorization", "DSNLogin concentrator=\"ctr\""); // valeur authorization non valide
        Invocation invocation = builder.buildGet();
        // When
        Response response = invocation.invoke();
        // Then
