...
@DisplayName("Rechercher Depots Par Declarant Msa Test")
class RechercherDepotsParDeclarantMsaTest extends BaseTestClass {
    ...
    void rechercher200ParDeclarantTest() {
        // Given
        ResteasyClient client = new ResteasyClientBuilder().disableTrustManager().build();
        ResteasyWebTarget target = client.target(getServerUrl() + "/lister-depots/1.0/" + getDateDebutFormatted(Config.plageDeRechercheParDeclarant));
        Builder builder = target.request();
    ...
    void rechercher401DECLARANTMauvaisTest() {
        ...
        map.put("declarant", Base64Utils.encode("Not a é valid token"));
        String authorization = SecurityUtils.buildAuthorizationHeader(map);
        builder.header("Authorization", authorization);
        Invocation invocation = builder.buildGet();
        // When
